...
def web_browser(driver_path, chrome_path, url="https://www.google.com", chrome_options=[]):
    ...
    #browser_scripts.verifyBrowserNDriverFiles(driver_path, chrome_path)

    options = Options() #instantiate Chrome Options
    service = Service(executable_path=driver_path) #instantiate Chrome Service         

    #this option ensures that we specify our own chrome browser
    #not the one installed natively on the windows
    options.binary_location = chrome_path #os.path.join(, "chrome.exe")
